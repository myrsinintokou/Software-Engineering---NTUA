...
//-----------------------Date-------------------------
router.get('/energy/api/AggregatedGenerationPerType/:areaname/:prodtype/:rescode/date/:date', verifyToken, (req, res) => {
    jwt.verify(req.token, 'test', async (err, authData) => {
        if (err) res.status(401).send('Not authorized')
            ...
            const prod = req.params.prodtype
            let format = req.query.format

            const querySt = `SELECT AreaName,AreaTypeCodeText,MapCodeText,ResolutionCodeText,
            Year,Month,Day,DateTime,ProductionTypeText,ActualGenerationOutput,UpdateTime 
      ...
      WHERE
          AreaName = ? AND DATE(DateTime)= '${date}' AND ResolutionCodeText =? AND ProductionTypeText=?
          ORDER BY DateTime ASC , ProductionTypeText ASC`;
            db.query(querySt, [area, resc, prod], (err, rows, fields) => {
                if (err) throw err
...
//-----------------------Month------------------------
router.get('/energy/api/AggregatedGenerationPerType/:areaname/:prodtype/:rescode/month/:date', verifyToken, (req, res) => {
    jwt.verify(req.token, 'test', async (err, authData) => {
        if (err) res.status(401).send('Not authorized')
        else {
            const uname = authData.username
      ...
      (SELECT DISTINCT Day, Month,AreaName,AreaTypeCodeText,MapCodeText,ResolutionCodeText,Year,ProductionTypeText
        ...
        INNER JOIN ProductionType p ON a.ProductionTypeId=p.Id
      WHERE
          AreaName = ? AND Month=MONTH('${date}') AND ResolutionCodeText = ? AND ProductionTypeText=?
...
//-----------------------Year-------------------------
router.get('/energy/api/AggregatedGenerationPerType/:areaname/:prodtype/:rescode/year/:date', verifyToken, (req, res) => {
    jwt.verify(req.token, 'test', async (err, authData) => {
        if (err) res.status(401).send('Not authorized')
        else {
            const uname = authData.username
      ...
      (SELECT DISTINCT Month, AreaName,AreaTypeCodeText,MapCodeText,ResolutionCodeText,Year,ProductionTypeText
        ...
        INNER JOIN ProductionType p ON a.ProductionTypeId=p.Id
      WHERE
          AreaName = ? AND Year=? AND ResolutionCodeText = ? AND ProductionTypeText=?
      ...
      ORDER BY Month ASC) s2 ON s1.Month=s2.Month AND s1.ProductionTypeText=s2.ProductionTypeText`;
            db.query(querySt, [area, date, resc, prod], (err, rows, fields) => {
                if (err) throw err
                const result = rows.map((row) => {
                    return {
                        Source: 'entso-e', Dataset: 'AggregatedGenerationPerType', AreaName: row.AreaName,
